  ...
  return ( // Zwracamy osobny widok koszyka podłączony w App.js do ścieżki /cart.
    <section> {/* Używamy sekcji, ponieważ koszyk jest samodzielnym widokiem aplikacji. */}
      ...
      {cartItems.length === 0 ? ( // Sprawdzamy, czy użytkownik nie dodał jeszcze żadnych produktów.
        <p data-testid="cart-empty">Koszyk jest pusty. Przejdź do produktów i dodaj wybrane pozycje.</p> // Pokazujemy komunikat dla pustego koszyka i oznaczamy go selektorem testowym.
        ...
        <div> {/* Grupujemy listę pozycji i podsumowanie koszyka. */}
          <ul> {/* Tworzymy listę produktów dodanych do koszyka. */}
            {cartItems.map((item, index) => ( // Iterujemy po pozycjach koszyka, ponieważ ten sam produkt może wystąpić kilka razy.
              <li key={`${item.id}-${index}`} data-testid="cart-item"> {/* Łączymy id produktu i indeks, aby klucz był unikalny także dla duplikatów i mierzalny w Selenium. */}
                ...
                <button type="button" data-testid={`remove-from-cart-${index}`} onClick={() => removeFromCart(index)} style={{ marginLeft: '10px' }}> {/* Dodajemy usuwanie pojedynczej pozycji bez mutowania tablicy stanu i z selektorem dla testu wielokartowego. */}
          ...
          <h3 data-testid="cart-total">Do zapłaty: {cartTotal.toFixed(2)} PLN</h3> {/* Pokazujemy sumę wyliczoną w AppContext na podstawie hooków Reacta i dostępną dla testu spójności. */}
