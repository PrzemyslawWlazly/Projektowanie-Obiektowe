...
func paymentsHandler(w http.ResponseWriter, r *http.Request) { // Definiujemy handler obsługujący endpoint płatności.
	...
	fmt.Println("Przyjęto poprawne żądanie płatności.") // Logujemy zdarzenie bez danych pochodzących od użytkownika.
...
func main() { // Definiujemy funkcję startową aplikacji serwerowej.
	...
	fmt.Println("Serwer Go działa na porcie 8080") // Wypisujemy informację ułatwiającą sprawdzenie, czy backend wystartował.
	log.Fatal(http.ListenAndServe(":8080", nil))   // Uruchamiamy serwer HTTP i kończymy program z błędem, jeśli start się nie powiedzie.
